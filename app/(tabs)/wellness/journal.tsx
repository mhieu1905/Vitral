...
const C = {
  bg: '#FDF8F3', surface: '#F5EFE6',
  sage: '#A8C5A0', sageD: '#6B9E62', sageL: '#D6E8D2',
      ...
      >
        ...
        <View style={s.header}>
          ...
          <View>
            ...
            <Text style={s.headerDate}>Tuesday, Jan 14</Text>
          </View>
        </View>
        ...
        >
          ...
          <View style={s.moodStrip}>
            {MOODS.map((emoji, i) => (
              <TouchableOpacity
                key={i}
                style={[s.moodCircle, i === mood && s.moodCircleSel]}
                onPress={() => setMood(i)}
                activeOpacity={0.8}
              >
          ...
          <View style={s.tagsRow}>
            {TAGS.map(tag => (
              <TouchableOpacity
                key={tag}
                style={[s.tag, selTags.includes(tag) && s.tagSel]}
                onPress={() => toggleTag(tag)}
                activeOpacity={0.8}
              >
                <Text style={[s.tagText, selTags.includes(tag) && s.tagTextSel]}>{tag}</Text>
              </TouchableOpacity>
            ))}
          </View>

          {/* ── Save ── */}
          <TouchableOpacity style={s.saveBtn} onPress={() => router.push('/(tabs)/wellness')} activeOpacity={0.85}>
...
const s = StyleSheet.create({
  ...
  header: {
    ...
    gap: 12, paddingHorizontal: 24, paddingTop: 12, paddingBottom: 12,
  },
  backBtn: {
  ...
  headerDate:  { fontSize: 11, color: C.muted, marginTop: 1 },

  content: { paddingHorizontal: 24, paddingBottom: 110 },

  moodStrip: {
    backgroundColor: C.white, borderRadius: 16,
    flexDirection: 'row', justifyContent: 'space-around',
    paddingVertical: 10, marginBottom: 14,
    shadowColor: '#3D3530', shadowOpacity: 0.06,
  ...
  moodCircleSel: { backgroundColor: '#D6E8D2', borderColor: C.sage },
  moodEmoji: { fontSize: 22 },

  textarea: {
    backgroundColor: C.white, borderWidth: 1, borderColor: C.surface,
    borderRadius: 16, padding: 14, fontSize: 14, color: C.dark,
    minHeight: 200, lineHeight: 22, marginBottom: 14,
  },
  ...
  tagSel:      { backgroundColor: C.sage },
  tagText:     { fontSize: 12, fontWeight: '500', color: C.muted },
  tagTextSel:  { color: C.white },
