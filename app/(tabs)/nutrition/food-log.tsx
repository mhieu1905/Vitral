    ...
    <SafeAreaView style={s.safe}>
      <ScrollView style={s.scroll} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        <View style={s.headerSection}>
          <View style={s.headerLeft}>
            ...
            <View style={s.amountRow}>
              <Text style={s.amountValue}>{FOOD_LOG_TOTAL.consumed.toLocaleString()}</Text>
              <Text style={s.amountTotal}>/ {FOOD_LOG_TOTAL.goal.toLocaleString()} kcal</Text>
            </View>
          </View>
        ...
        <View style={s.progressBg}>
          <View style={[s.progressFill, { width: `${FOOD_LOG_TOTAL.pct * 100}%` }]} />
        </View>
            ...
            {FOOD_LOG_MACRO_PIES.map((m) => (
              <View key={m.label} style={s.macroItem}>
                ...
                <View style={s.macroItemBarBg}>
                  <View style={[s.macroItemBarFill, { width: `${m.pct * 100}%` }]} />
                </View>
              </View>
            ))}
          </View>
...
const s = StyleSheet.create({
  ...
  amountRow: { marginTop: 4, flexDirection: 'row', alignItems: 'flex-end', gap: 8 },
  amountValue: { fontFamily: f.displayBold, fontSize: 36, color: c.sageDark, lineHeight: 40 },
  amountTotal: { fontFamily: f.display, fontSize: 18, color: 'rgba(75,101,70,0.4)', lineHeight: 28, marginBottom: 2 },
  leftBadge: {
    paddingVertical: 4,
  ...
  leftBadgeText: { fontFamily: f.displaySemi, fontSize: 12, color: c.sageDark, lineHeight: 16 },
  ...
  addDinnerText: { fontFamily: f.displayBold, fontSize: 14, color: '#FFFFFF', lineHeight: 20 },
  ...
  macroGrid: { marginTop: 40, flexDirection: 'row', justifyContent: 'space-between', gap: 32 },
  macroItem: { flex: 1, alignItems: 'center', gap: 16 },
  macroItemLabel: { fontFamily: f.displaySemi, fontSize: 12, color: c.sage, lineHeight: 16 },
  macroItemValue: {
    marginTop: -12,
    fontFamily: f.displayBold,
    fontSize: 20,
